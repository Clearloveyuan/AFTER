...
def similarity(centers, vectors, normalized):
...
def get_nearest_to_centers(centers, vectors, normalized=True):
    sim = similarity(centers, vectors, normalized)
    return sim.argmax(axis=1)



def get_nearest_to_centers_iterative(cluster_centers, vectors, normalized=True):
    indices = np.empty(cluster_centers.shape[0], dtype=int)
    for i in range(cluster_centers.shape[0]):
        sim = similarity(cluster_centers[None, i], vectors, normalized)
